...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    // Sanitize userId for sheet name
    const safeTitle = String(userId).replace(/[@.]/g, '_').substring(0, 90);

    try {
        const doc = await getDoc();
        let sheet = doc.sheetsByTitle[safeTitle];
        
        // GET: Return list of links
        if (req.method === 'GET') {
            if (!sheet) return res.status(200).json([]);
            const rows = await sheet.getRows();
            const links = rows.map(r => r.get('URL')).filter(Boolean);
            return res.status(200).json(links);
        }

        // POST/DELETE needs sheet to exist
        if (!sheet) {
            sheet = await doc.addSheet({ title: safeTitle, headerValues: ['Title', 'URL', 'Date', 'AddedAt'] });
        }
        ...
        if (req.method === 'POST') {
            ...
            // Allow duplicates? No. Check if exists.
            const rows = await sheet.getRows();
            const exists = rows.find(r => r.get('URL') === link);
            if (!exists) {
        ...
        if (req.method === 'DELETE') {
            const { link } = req.body;
            const rows = await sheet.getRows();
            const rowToDelete = rows.find(r => r.get('URL') === link);
            if (rowToDelete) {
