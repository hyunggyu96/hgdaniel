...
export async function GET(request: Request) {
    ...
    if (companyName) {
        const sanitizedName = companyName.replace(/[%_\\]/g, '\\$&').slice(0, 200);
        dbQuery = dbQuery.ilike('company_name', `%${sanitizedName}%`);
    }
    if (status === 'active') {
    ...
    if (error) {
        console.error('[nedrug-products] Query error:', error);
