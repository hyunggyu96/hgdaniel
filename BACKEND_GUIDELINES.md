...
- **화이트리스트 강제(Whitelist Only)**: AI가 추출한 키워드 중 `EXPERT_ANALYSIS_KEYWORDS`(풀)에 존재하지 않는 검색어는 코드 단에서 즉시 폐기합니다. (환각 방지 핵심)
- **요약문 정제(Summary Tail-cleaning)**: 문장 끝의 불필요한 마침표(`.`), 깨진 조사 및 미완성 단어를 자동 제거하는 후처리 로직을 적용합니다.
- **Few-shot 프롬프팅**: AI에게 '올바른 추출물'과 '잘못된 추출물'의 예시를 대조하여 학습시킴으로써 남발을 방지합니다.
