...
- **BAD_KEYWORDS 확장**
  - 세금/조세: `OECD최저세`, `글로벌최저한세`, `조세회피`, `미 재무부`, `JP모건`
  - 금융/증시: `코스피`, `코스닥`, `증시`, `주가지수`
  - DB에서 95개 노이즈 기사 삭제

- **정확 매칭 키워드 (EXACT_MATCH_KEYWORDS)**
  - `스킨부스터`: "스킨부스터" 또는 "스킨 부스터" 정확 매칭 필수
  - "스킨" + "부스터" 따로 있으면 노이즈로 필터링

- **기업명 단독 분류 (COMPANY_ONLY_KEYWORDS)**
  - 파마리서치, 휴젤 등 기업명만 있으면 → Corporate News
  - 제품 키워드(필러, 톡신 등) 없을 때 적용
  - DB에서 288개 기사 재분류

### 📁 문서 정리

- 루트 디렉토리 58개 → 7개 스크립트 정리
- `ARCHITECTURE.md`, `PROJECT_STRUCTURE.md` 삭제 (중복)
- `❗TABLET_WORK_PENDING.md` 삭제 (완료됨)
- `_docs/README.md` 문서 구조 정리
...
- **2026-01-06**: 노이즈 필터링 V2.0 (세금/증시/기업명 분류)
