...
- **모델**: **Qwen 2.5 3B** (qwen3b.gguf)
- **포트**: `8080` ⚠️
- **가속 설정**:
  - `gpu-layers`: **`-ngl 0`** (CPU Only - 안정성 최우선)
  - `threads`: **`-t 4`** (Snapdragon 고성능 코어 4개만 사용 必 - `-t 8` 금지 🚫)
  - `context`: **`-c 4096`** (긴 뉴스 에러 방지 - `2048` 금지 🚫)
  - `batch`: `-b 128 -ub 64` (반응성 향상)

### 🧹 데이터 필터링 정책 (Noise Filtering)

1. **즉시 차단 (Blacklist)**:
    - **리워드 앱**: 캐시워크, 돈버는퀴즈, 정답 등 (AI 분석 전 폐기)
    - **자동차/디스플레이**: `패스트백`, `LG디스플레이`, `필러투필러` 등 명백한 노이즈.
2. **스마트 필터 (Smart Regex)**:
    - **자동차 필러**: `A-필러`, `B필러` 등은 **자동차 기둥(Column)** 문맥일 때만 차단.
    - **보호 대상**: `HA필러`(히알루론산), `PLLA필러` 등 **의료 용어는 정상 수집** ✅.
    - **원리**: 단순 단어 일치가 아닌, **앞글자 패턴**(`[A-C] + 필러`)을 확인하여 정밀 타격.
