...
create table if not exists companies (
    ...
    -- 'KOSPI', 'KOSDAQ', 'Unlisted', 'Global_Listed', 'Global_Private'
...
do $$ begin drop policy "Enable read access for all users" on companies;
exception
when undefined_object then
end $$;
create policy "Enable read access for all users" on companies for
select using (true);
-- 3. 기존 데이터 입력 (기본값)
...
select d.id,
    ...
    d.category
from (
        values (1, '한스바이오메드', 'HansBiomed', 'KOSDAQ', 'korean'),
