...
# API Regulation: 10 req/sec with API key. User asked for 8.
# 1 / 8 = 0.125 seconds
...
def process_existing_papers(supabase: Client, pmids: List[str]):
    ...
    If they exist, ensure 'HIFU' is in their keywords.
...
def main():
    print(f"=== PubMed Fetch for '{TARGET_KEYWORD}' ===")
    ...
    # 1. Fetch all IDs
    pmids = fetch_pubmed_ids(TARGET_KEYWORD, TARGET_COUNT)
    print(f"  Found {len(pmids)} IDs for '{TARGET_KEYWORD}'.")
